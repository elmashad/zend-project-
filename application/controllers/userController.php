...
class UserController extends Zend_Controller_Action
{
    ...
    public function createAction()
    {
        ...
        if ($this->_request->isPost()) {
            if ($userForm->isValid($_POST)) {
                ...
                $userModel->createUser($userForm->getValue('username'), $userForm->getValue('password'),
                    $userForm->getValue('first_name'), $userForm->getValue('last_name'), $userForm->
                    getValue('role'));
                $userForm->setAction('/zendf/user/create');
            }
        }
        $userForm->setAction('/zendf/user/create');
    ...
    {
        $userForm = new Form_User();
        $userForm->setAction('/zendf/user/update');
        $userForm->removeElement('password');
        $userModel = new Model_User();
        if ($this->_request->isPost()) {
            if ($userForm->isValid($_POST)) {
                $userModel->updateUser($userForm->getValue('id'), $userForm->getValue('username'),
                    $userForm->getValue('first_name'), $userForm->getValue('last_name'), $userForm->
                    getValue('role'));
    ...
    public function passwordAction()
    {
        ...
        $passwordForm->setAction('/zendf/user/password');
        ...
        if ($this->_request->isPost()) {
            if ($passwordForm->isValid($_POST)) {
                $userModel->updatePassword($passwordForm->getValue('id'), $passwordForm->
                    getValue('password'));
                return $this->_redirect('/user/list');
            }
        } else {
    ...
    {
        $userForm = new Form_User();
        $userForm->setAction('/zendf/user/login');
        ...
        if ($this->_request->isPost() && $userForm->isValid($_POST)) {
            ...
            $authAdapter = new Zend_Auth_Adapter_DbTable($db, 'users', 'username',
                'password');
            ...
            if ($result->isValid()) {
                ...
                $storage->write($authAdapter->getResultRowObject(array('username', 'first_name',
                    'last_name', 'role')));
